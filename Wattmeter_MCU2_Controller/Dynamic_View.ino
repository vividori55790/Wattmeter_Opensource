 ...
 * 버전: v101 (Serial1 Compatible)
 * 설명: MCU2 직접 ADC 샘플링 파형 + Serial1 기반 데이터 갱신.
 * ==============================================================================
 */
...
void updateYAxisLabels() {
  ...
  if (waveformPlotType == 0) { // V/I
    ...
    tft.setCursor(0, PLOT_Y_START + 5); tft.print("+"); tft.print(buffer); tft.print("Var"); 
    tft.setCursor(0, PLOT_Y_END - 10); tft.print("-"); tft.print(buffer); tft.print("Var");
  } else { // I/I1/I2
    tft.setTextColor(COLOR_TEXT_PRIMARY); 
...
void runCombinedWaveformLoop() {
  ...
  for (int i = 0; i < PLOT_WIDTH; i++) {
    checkTouchInput(); 
    checkSerialInput(); // MCU1 데이터 수신 확인 (Serial1)
  ...
  if (waveformPlotType == 0) { // V/I
    ...
    new_p2_axis = findAxisStep(new_frame_p2_peak, P_AXIS_STEPS, NUM_P_STEPS); 
  } else { // I/I1/I2
...
void displaySettingsTimerValues() {
  if (temp_timer_setting_seconds != prev_temp_timer_setting_seconds) {
    ...
    char buffer[15]; sprintf(buffer, "%02d:%02d:%02d", hours, minutes, seconds);
    tft.setTextSize(2); 
    int x_pos = 110; int y_pos = 80;  
    if (prev_temp_timer_setting_seconds != 0xFFFFFFFF) {
        uint32_t prev_total = prev_temp_timer_setting_seconds;
        uint16_t ph = prev_total / 3600; uint16_t pm = (prev_total % 3600) / 60; uint8_t ps = prev_total % 60;
        char prev_buffer[15]; sprintf(prev_buffer, "%02d:%02d:%02d", ph, pm, ps);
        if (strcmp(buffer, prev_buffer) != 0) {
           tft.setTextColor(COLOR_BACKGROUND); tft.setCursor(x_pos, y_pos); tft.print(prev_buffer);
        }
    }
    tft.setTextColor(COLOR_ORANGE); tft.setCursor(x_pos, y_pos); tft.print(buffer);
  ...
  if (is_timer_active != prev_is_timer_active) {
    if (is_timer_active) {
       ...
       tft.setCursor(245, 130); tft.print("STOP");
    } else {
       tft.fillRoundRect(230, 115, 80, 50, 8, COLOR_BUTTON); 
       tft.drawRoundRect(230, 115, 80, 50, 8, COLOR_BUTTON_OUTLINE);
       tft.setTextColor(COLOR_BUTTON_TEXT); tft.setTextSize(2);
       tft.setCursor(240, 130); tft.print("START");
    }
  }
  ...
  if (timer_seconds_left != prev_timer_seconds_left) {
    uint16_t hours = timer_seconds_left / 3600; uint16_t minutes = (timer_seconds_left % 3600) / 60; uint8_t seconds = timer_seconds_left % 60;
    char buffer[15]; sprintf(buffer, "%02d:%02d:%02d", hours, minutes, seconds);
    char prev_buffer[15]; uint16_t ph = prev_timer_seconds_left / 3600; uint16_t pm = (prev_timer_seconds_left % 3600) / 60; uint8_t ps = prev_timer_seconds_left % 60;
